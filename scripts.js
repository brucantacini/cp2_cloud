...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  function applyFilter(){
    ...
    cards.forEach(card=>{
      ...
      if(matchesQ && matchesC){card.classList.remove('hidden');visible++;} else {card.classList.add('hidden');}
    });
    return visible;
  }

  search.addEventListener('input', ()=>{applyFilter(); highlightSearch();});
  ...
  function highlightSearch(){
    ...
    cards.forEach(card=>{
      ...
      } else { titleEl.classList.remove('search-highlight'); }
    });
  }

  randomBtn.addEventListener('click', ()=>{
    const visibleCards = cards.filter(c=>!c.classList.contains('hidden'));
    if(visibleCards.length===0){ alert('Nenhuma receita encontrada para sortear.'); return; }
    const pick = visibleCards[Math.floor(Math.random()*visibleCards.length)];
    pick.scrollIntoView({behavior:'smooth',block:'center'});
    pick.animate([{transform:'scale(1)'},{transform:'scale(1.03)'},{transform:'scale(1)'}],{duration:420});
    // breve destaque
